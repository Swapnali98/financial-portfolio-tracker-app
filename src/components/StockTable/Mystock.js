...
class Mystock extends Component {
  ...
  componentDidMount() {
    axios
      .get(`https://finance-portfolio-tracke-7675f.firebaseio.com/mystock1.json`)
      .then((db) => {
        const res = db.data;
        if (res === null) {
          this.setState({
          ...
          for (let i = 0; i < Keys.length; i++) {
            ...
            this.StockInfo = (x) => {
              axios
                .get(
                  `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${c_symb}&apikey=ZQNQ3PAOY2KRLEL0`
                )
                .then((res) => {
                  let Data = [];
                  let Current_p = [];
  ...
  stopTrack = (index) => {
    ...
    let ref = firebase.database().ref("mystock1");
    ref
